...
def caffe_to_keras(prototext, caffemodel, phase='train', debug=True):
        '''
        ...
        print("CREATING MODEL", config.input_dim )
...
def create_model(layers, phase, input_dim, debug=False):
    ...
    for layer_nb in network:
        ...
        if layer_nb in inputs:
            ...
            # print(dim,input_dim, get_data_dim(layers[0]))
        ...
        # other cases
            ...
            if type_of_layer == 'concat':
                ...
                net_node[layer_nb] = Deconvolution2D(nb_filter, nb_row, nb_col, bias=has_bias, subsample=(8, 8), name=name)(input_layers)


                                    #  strides=(1, 1), padding='valid', data_format=None, activation=None, use_bias=True,
                ...
                if pad_h + pad_w > 0:
                    input_layers = ZeroPadding2D(padding=(pad_h, pad_w), name=name + '_zeropadding',dim_ordering="th")(input_layers)
                    input_layer_name = name + '_zeropadding'
                if(layer.pooling_param.pool == 0): # MAX pooling
                    #border_mode = 'same'
                    border_mode = 'valid'
                    net_node[layer_nb] = MaxPooling2D(border_mode=border_mode, pool_size=(kernel_h, kernel_w), strides=(stride_h, stride_w), name=name,dim_ordering="th")(input_layers)
                    if(debug):
                        print("MAX pooling")
                elif(layer.pooling_param.pool == 1): # AVE pooling
                    net_node[layer_nb] = AveragePooling2D(pool_size=(kernel_h, kernel_w), strides=(stride_h, stride_w), name=name,dim_ordering="th")(input_layers)
                    if(debug):
                        print("AVE pooling")
                else: # STOCHASTIC?
                ...
                r = Reshape(1, 100, 100)#   (input_layers[0])
                net_node[layer_nb] = Activation('softmax', name=name)(input_layers)
                # net_node[layer_nb] = Activation('softmax', name=name)
